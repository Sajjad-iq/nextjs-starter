...
export function DatePickerField({ field, value, onChange, isLoading }: DatePickerFieldProps) {
  ...
  const [inputValue, setInputValue] = React.useState(formatDate(parsedDate, field.dateFormat))
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (isValidDate(date)) {
                ...
                onChange(field.dateFormat === "iso" ? formatDate(date, "iso") : date)
                setMonth(date)
              }
            }}
            onKeyDown={(e) => {
